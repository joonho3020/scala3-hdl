...
type NTOf[T] <: NamedTuple.AnyNamedTuple = T match
  ...
  case _           => NamedTuple.NamedTuple[ShapeOf[T]#Labels, ShapeOf[T]#Elems]
...
// val s = summonInline[ShapeOf[MyBundle]]
// println(s"s.labels ${s.labels}")

  val reg = Reg(mb)
  println(showTypeOf[Reg[MyBundle]#Fields])
