...
object Bundle:
  ...
  transparent inline def lit(inline elems: (String, Signal)*): Any =
    ${ BundleMacros.bundleLitFromCompImpl('this, 'elems) }

  extension (companion: Any)
    transparent inline def lit(inline elems: (String, Signal)*): Any =
      ${ BundleMacros.bundleLitFromCompImpl('companion, 'elems) }
