...
- [x] Per-class caching??
    ...
    - This may make things more fragile. Better to be pessemistic
- [x] Bulk connections
...
## Additional Features
 ...
 Fill(n: Int, x: Bool): UInt
    - Returns a UInt with n bits of x
- Reverse(x: UInt): UInt
    - Reverses the bits of x

- DontCare support for HWEnums?

When we have the following enum:
...
```scala
val x: HWEnum[Immediates] = Wire(Immediates.E)
x := Immediates.DontCare.EN
```

Here, `DontCare` isn't necessarily another case that doesn't match with the existing enums.
Rather, it can match with `IMM_S`, `IMM_SB`, ... `IMM_Z`.

- BitPat:
    - `https://github.com/chipsalliance/chisel/blob/v7.5.0/src/main/scala/chisel3/util/BitPat.scala#L294`
    - Would like to use this to generate decode tables like the following
...
```scala
switch (inst.asBitPat) {
    is (ADD) { CtrlSignals(true.B, Immediates.IMM_I.N, ... ) }
    ...
}
```
