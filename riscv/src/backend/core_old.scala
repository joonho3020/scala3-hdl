...
class Core(p: CoreParams) extends Module with CoreCacheable(p):
  ...
  body {
    ...
    for (i <- 0 until coreWidth) {
      when (!ex_stall) {
        when (dec_uops(i).valid && dec_ready(i) && !ex_clear) {
          ...
          ex_rs1_rf(i) := rf(dec_uops(i).bits.lrs1)
    ...
    bpu_update.bits.is_call := (cfi_uop.bits.ctrl.jal || cfi_uop.bits.ctrl.jalr) && cfi_uop.bits.lrd === 1.U
    bpu_update.bits.is_ret  := cfi_uop.bits.ctrl.jalr && cfi_uop.bits.lrs1 === 1.U && cfi_uop.bits.lrd === 0.U
